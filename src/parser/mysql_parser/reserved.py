...
sql92_reserved = (
    ...
    'ORDER', 'HAVING', 'AT', 'XOR', 'OR', 'AND', 'IN', 'NOT', 'NO', 'EXISTS', 'BETWEEN', 'LIKE', 'IS',
...
sql99_reserved = (
    ...
    'NATURAL', 'NCHAR', 'NCLOB', 'NESTING', 'NEW', 'NEXT', 'NO', 'NONE', 'NOT', 'NOWAIT', 'NULL', 'NUMERIC',
    ...
    'VIEW', 'WHEN', 'WHENEVER', 'WHERE', 'WHILE', 'WITH', 'WITHOUT', 'WORK', 'WRITE', 'XOR', 'YEAR'
)
...
presto_tokens = (
    ...
    'BETWEEN', 'BIGINT', 'BINARY', 'BLOB', 'BOTH', 'BY', 'BINLOG',
    'CALL', 'CASCADE', 'CASE', 'CHANGE', 'CHARACTER', 'CHAR', 'CHECK', 
    'COLLATE', 'COLUMN', 'CONSTRAINT', 'CONTINUE', 'CONVERT', 'CREATE', 
    'CROSS', 'CUME_DIST', 'CURRENT_DATE', 'CURRENT_TIME', 'CURRENT_TIMESTAMP', 
    'CURRENT_USER', 'CURRENT_ROLE', 'CURSOR',
    'DATABASE', 'DATABASES', 'DAY_HOUR', 'DAY_MICROSECOND', 'DAY_MINUTE',
    'DAY_SECOND', 'DECIMAL', 'DEFAULT', 'DELAYED', 'DELETE', 'DENSE_RANK', 
    'DESC', 'DESCRIBE', 'DISTINCT', 'DISTINCTROW', 'DIV', 'DOUBLE', 'DROP', 'DUAL',
    'ELSEIF', 'ELSE', 'ENCLOSED', 'ESCAPED', 'EXISTS', 'EXIT', 'EXPLAIN', 'EXCEPT',
    'FALSE', 'FETCH', 'FIRST_VALUE', 'FLOAT', 'FOR', 'FORCE', 'FOREIGN',  'FULLTEXT',
    'GENERATED', 'GRANT',  'GROUPS', 
    'HAVING', 'HIGH_PRIORITY', 'HOUR_MICROSECOND', 'HOUR_MINUTE', 'HOUR_SECOND', 
    'IGNORE',  'INDEX', 'INFILE', 'INNER', 'INOUT', 'INTEGER', 'INTERSECT', 
    'INTERVAL', 'OUTFILE', 'INSERT', 'INT', 'INT1', 'INT2', 'INT3', 
    'INT4', 'INT8', 'ITERATE', 
    'JOIN', 
    'KEY', 'KEYS', 'KILL', 
    'LAG', 'LAST_VALUE', 'LEAD', 'LEADING', 'LEAVE', 'LEFT', 'LIKE', 'ILIKE', 'LIMIT', 
    'LINES', 'LINEAR', 'LOAD', 'LOCALTIME', 'LOCALTIMESTAMP', 'LOCK','LONG','LONGBLOB',
    'LONGTEXT', 'LOW_PRIORITY', 
    'MATCH', 'MAXVALUE', 'MEDIUMBLOB', 'MEDIUMINT', 'MEDIUMTEXT', 'MINUTE_MICROSECOND', 
    'MINUTE_SECOND', 'MOD',
    'NATURAL','NOT', 'NO_WRITE_TO_BINLOG', 'NTH_VALUE', 'NTILE', 'NULL', 'NUMERIC',
    'OF', 'ON', 'OPTIMIZE', 'OPTION', 'OPTIONALLY', 'ORDER', 'OUT', 'OUTER', 'OVER', 
    'PARTITION', 'PERCENT_RANK', 'PRECISION', 'PRIMARY', 'PROCEDURE', 
    'RANGE', 'RANK', 'READ', 'REAL', 'RECURSIVE', 'REFERENCES', 'REGEXP', 'RELEASE', 'RETURN','RETURNED_SQLSTATE',
    'RETURNS','RENAME', 'REPEAT', 'REPLACE','RESIGNAL','REQUIRE', 'RESTRICT', 'REVOKE', 'RIGHT', 'RLIKE', 
    'ROW', 'ROWS', 'ROW_COUNT',
    'SECOND_MICROSECOND', 'SELECT', 'SET', 'SHOW','SIGNAL','SMALLINT', 'SPATIAL', 'SQL', 'SQL_BIG_RESULT',
    'SQL_CALC_FOUND_ROWS','STORED', 'SQL_SMALL_RESULT', 'SQLEXCEPTION', 'SQLSTATE', 'SQLWARNING', 'SSL', 
    'STARTING', 'STATS_EXTENDED', 'STRAIGHT_JOIN', 'STRING',
    'TABLE', 'TABLESAMPLE', 'TERMINATED', 'THEN', 'TINYBLOB', 'TINYINT', 'TINYTEXT', 
    'TO', 'TRAILING', 'TRIGGER', 'TRUE', 
    'UNIQUE', 'UNION', 'UNLOCK', 'UNSIGNED', 'UNTIL', 'UPDATE', 'USAGE', 
    'USING', 'UTC_DATE', 'UTC_TIMESTAMP', 'UTC_TIME', 'VALUES', 
    'VARCHAR', 'VARCHACTER', 'VARBINARY', 'VARYING', 'VIRTUAL', 
    'WHEN', 'WHERE', 'WHILE', 
    'WRITE','XOR', 
    'YEAR_MONTH', 
    'ZEROFILL', 
...
presto_nonreserved = (
    'ACCOUNT', 'ACTION', 'ADVISE', 'AFTER', 'AGAINST', 'AGO', 'ALGORITHM', 
    'ALWAYS', 'ANY','AVG', 'AVG_ROW_LENGTH', 'AUTO_INCREMENT',
    'BACKUP', 'BEGIN','BIT','BOOL','BOOLEAN','BTREE','BYTE', 
    'CACHE', 'CALL', 'CAPTURE', 'CASCADED', 'CAUSAL', 'CHARSET', 'CLEANUP', 'CLIENT', 'COALESCE',
    'COLLATION', 'COLUMN', 'COLUMN_FORMAT', 'COLUMNS', 'COMMENT', 'COMMIT','COMMITTED','COMPACT', 'COMPRESSED', 
    'COMPRESSION', 'CONCURRENCY', 'CONNECTION', 'CONSISTENCY', 'CONSISTENT', 'CONTEXT', 'CPU','CURRENT', 'CYCLE', 
    'DATA', 'DATE', 'DATETIME', 'DAY', 'DECLARE',  'DISABLE', 'DISABLED', 'DISCARD', 'DISK', 'DO', 'DUPLICATE', 'DYNAMIC', 
    'ENABLE', 'ENABLED', 'END', 'ENFORCED', 'ENGINE', 'ENUM', 'ERROR', 'ESCAPE', 
    'EVENT', 'EVENTS', 'EXECUTE', 'EXTENDED', 
    'FIELDS', 'FILE','FIRST','FLUSH','FOR', 'FORMAT', 'FOUND', 'FULL', 'FUNCTION', 'FROM',
    'GENERAL','GLOBAL', 'GRANTS','GROUP', 
    'HANDLER', 'HASH', 'HELP','HOSTS','HOUR', 
    'IDENTIFIED', 'IMPORT','IN','INTO','INDEXES', 'INSERT_METHOD', 'INSTANCE', 'INVOKER',
    'INVISIBLE', 'IO', 'IPC', 'IS', 'ISOLATION', 'ISSUER', 'ITERATE', 
    'JSON', 
    'KEY_BLOCK_SIZE',  
    'LANGUAGE', 'LAST','LATERAL', 'LESS', 'LEVEL',  'LIST', 'LOCAL', 'LOCATION', 'LOCKED', 'LOGS', 
    'MASTER', 'MAX_ROWS', 'MB', 'MEMORY', 'MERGE', 'MICROSECOND','MIN_ROWS', 
    'MINUTE', 'MINVALUE', 'MODE', 'MODIFY', 'MONTH', 
    'NAMES', 'NATIONAL','NEVER', 'NEXT', 'NOWAIT', 'NODEGROUP', 'NONE', 'NULLS','NVARCHAR', 
    'OF', 'OFF', 'OFFSET','OR','ON_DUPLICATE', 'ONLINE', 'ONLY', 'OPEN', 'OPTIONAL', 
    'PACK_KEYS', 'PAGE', 'PARAMETER', 'PARSER', 'PARTIAL', 'PARTITION', 'PARTITIONING', 'PARTITIONS', 
    'PASSWORD', 'PAUSE', 'PERCENT', 'PER_DB', 'PER_TABLE', 'PLUGINS', 'POINT', 'POLICY', 
    'PRECEDING', 'PRESERVE', 'PRIVILEGES', 'PROCESSLIST', 'PROFILE', 'PROXY', 'PURGE', 
    'QUARTER', 'QUERY','QUICK', 
    'READS','REBUILD', 'RECOVER', 'REDOFILE','REDUNDANT', 'RELOAD', 'REMOVE','REORGANIZE','REPAIR', 
    'REPLACE', 'REPLICATE', 'REPLICATION', 'RESET', 'RESOURCE', 'RESPECT', 'REVERSE','RESTORE', 'RESUME', 'RETURN', 
    'RETURNS', 'ROLLBACK',  'ROW_FORMAT', 'RTREE',
    'SAVEPOINT','SECOND','SERIALIZABLE', 'SESSION','SHARE', 
    'SHUTDOWN', 'SIGNED', 'SIMPLE', 'SLAVE', 'SLOW', 'SNAPSHOT',  'SOME', 
    'SONAME', 'SOUNDS', 'SOURCE', 'SPATIAL', 'SPECIFIC', 'SQL_CALC_FOUND_ROWS', 
    'SQL_AFTER_GTIDS','SQL_AFTER_MTS_GAPS'
    'SQL_BEFORE_GTIDS','SQL_BUFFER_RESULT',
    'SQL_CACHE', 'SQL_NO_CACHE', 'SQL_SMALL_RESULT', 'SQL_THREAD', 'STACKED', 'START', 
    'STATS_AUTO_RECALC', 'STATS_PERSISTENT', 'STATS_SAMPLE_PAGES', 'STATUS', 'STOP', 
    'STORAGE', 'SUBJECT','STORED', 'STRAIGHT_JOIN', 'SUBCLASS_ORIGIN', 
    'SUPER', 'SUSPEND',
    'TABLES','TABLESPACE','TABLE_CHECKSUM','TEMPORARY', 'TEMPTABLE', 'TIME', 'TIMESTAMP', 
    'TRANSACTION', 'TRIGGERS', 'TRUNCATE', 'TYPE', 'TYPES',
    'UNCOMMITTED', 'UNDEFINED', 'UNDO', 'UNDO_BUFFER_SIZE', 'UNINSTALL', 'UNKNOWN','UNLOCK', 'USE','USER',
    'VALIDATION', 'VALUE', 'VALUES', 'VARIABLES', 
    'WAIT', 'WARNINGS', 'WEEK', 'WEIGHT_STRING', 'WITH', 'WITHOUT', 'WORK', 'WRAPPER', 'WRITE', 
    'X509', 'XA', 'XID', 'XML', 'XOR',
    'YEAR', 
    'ZEROFILL'
)
...
presto_not_keyword_token=(
    "ADDDATE",
    "BRIEF",
    "CAST","COPY","CURTIME","CURDATE",
    "DATE_ADD","DATE_SUB",
    "EXTRACT","GET_FORMAT","GROUP_CONCAT","MIN","MAX","NOW","POSITION",
