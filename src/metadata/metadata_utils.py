...
class MetaDataUtils(object):
    ...
    def schema_sql_to_catalog_index(schema_sql):
        ...
        for ddl_sql in schema_sql.split(';'):
            ...
            for _index_type, _index_name, _index_column_list in index_list:
                catalog_index_list.append(
                    Index(_index_name, _index_column_list, _index_type)
                )

            for _column_name, _column_type, _nullable in element_list:
                catalog_column_list.append(
                    Column(_column_name, _column_type, _nullable)
                )

            catalog_statistics_list.append(Statistics(None, table_name, []))
            catalog_table_list.append(
                Table(None, table_name, catalog_column_list, catalog_index_list, None)
            )
    ...
    def json_to_catalog(catalog_json, schema_sql=None) -> Catalog:
        ...
        if 'manually_statistics' in catalog_json:
            ...
            data_list = sorted(
                statistics_list, key=lambda item: (item['schema'], item['table'])
            )
            for (schema, table), group_data in itertools.groupby(
                data_list, key=lambda item: (item['schema'], item['table'])
            ):
                catalog_selectivity_list = []
                catalog_column_list = []
                catalog_index_list = []
                for per_plan in group_data:
                    catalog_selectivity_list.append(
                        Selectivity(
                            per_plan['name'], None, None, per_plan['cardinality']
                        )
                    )
                    catalog_column_list.append(Column(per_plan['name'], None, None))
                catalog_table_list_manually.append(
                    Table(schema, table, catalog_column_list, catalog_index_list, 100)
                )
                catalog_statistics_list.append(
                    Statistics(schema, table, catalog_selectivity_list)
                )
            if schema_sql:
                catalog_index_list = []
                catalog_column_list = []
            ...
            for table in table_list:
                ...
                rows = table['rows']
                schema_index_list = list(
                    filter(lambda x: (x['table'] == table_name), index_list)
                )
                schema_column_list = list(
                    filter(lambda x: (x['table'] == table_name), column_list)
                )
                first_uk = False
                column_set = set()
                index_name_set = set()
                for index in schema_index_list:
                    if index['column'] not in column_set:
                        ...
                        catalog_selectivity_list.append(
                            Selectivity(
                                index['column'], None, None, index['cardinality']
                            )
                        )
                ...
                catalog_statistics_list.append(
                    Statistics(schema_name, table_name, catalog_selectivity_list)
                )
                catalog_table_list.append(
                    Table(
                        schema_name,
    ...
    @staticmethod
    def is_index_back(
        idx_column_list: List,
        filter_column_list: List,
        projection_column_list: List,
        order_list: List,
    ...
    @staticmethod
    def has_interesting_order(
        idx_column_list: List,
        order_list: List,
        min_max_list: List,
        extract_range_list: List,
        filter_column_list: List,
    ) -> bool:
        """
        interesting order
    ...
    def index_all_match(idx_column_list: List, filter_column_list: List) -> bool:
        ...
        if (
            not filter_column_name_list
            or not filter_column_opt_list
            or not idx_column_list
        ):
            return False
            ...
            if _index_column not in filter_column_name_list:
                ...
                for _i, _filter_column in enumerate(filter_column_name_list):
                    if _index_column == _filter_column:
                        ...
                        if (
                            filter_column_opt_list[_i] != '='
                            and filter_column_opt_list[_i] != 'in'
                            and filter_column_opt_list[_i] != 'is'
                        ):
                            return False
    ...
    @staticmethod
    def get_column_opt_for_cost(
        column_name, filter_column_list, filter_column_opt_list
    ) -> OptType:
        """
        ...
        if cnt == 1:
            ...
            while cnt > 0:
                ...
                if opt == '=':
                    ...
                    opt == '<='
                    and opt_type.value <= OptType.CLOSED_RANGE_HALF_EQUAL.value
                ):
                    le = True
                    if gt:
                        opt_type = OptType.CLOSED_RANGE_HALF_EQUAL
                    ...
                    opt == 'like_prefix' or opt == 'between'
