...
from src.metadata.catalog import Catalog, Index
...
class Optimizer(object):
    def optimize(self, sql, catalog: Optional[Catalog], engine=OceanBaseEngine()):
        ...
        after_sql_rewrite, rewrite_rule_explanation_list = engine.rewrite(
            statement, catalog
        )
        development_specification_recommendation_list = engine.pmd(
            copy.deepcopy(statement)
        )
        after_sql_rewrite_format = (
            format_sql(after_sql_rewrite, 0) if rewrite_rule_explanation_list else None
        )

        rewrite_result = RewriteResult(
            after_sql_rewrite_format, rewrite_rule_explanation_list
        )
        ...
        for _table in table_list:
            ...
            _index_list = list(
                filter(lambda x: (x.table_name == _table['table_name']), index_list)
            )
            for i, _left in enumerate(_index_list):
                ...
                for _right in _index_list[i + 1 :]:
                    if _right.index_name in pruning_index_list:
                        continue
                    compare_stat = IndexPrunning(
                        _left, _right, len(_table['filter_column_list'])
                    ).skyline_compare()
            ...
            if heuristic_rule_return_result and heuristic_rule_return_result.index_name:
                index_optimization_recommendation_list.append(
                    {
                        'index_recommendation': self.index_optimization_recommendation_return_format(
                            heuristic_rule_return_result.index_name,
                            heuristic_rule_return_result.index_column_list,
                        ),
                        'diagnosis_reason': self.get_diagnosis_reason_by_index(
                            heuristic_rule_return_result.index_name, index_list
                        ),
                    }
                )
            else:
                ...
                if recommend_index:
                    index_optimization_recommendation_list.append(
                        {
                            'index_recommendation': self.index_optimization_recommendation_return_format(
                                recommend_index, recommend_index_column
                            ),
                            'diagnosis_reason': self.get_diagnosis_reason_by_index(
                                recommend_index, index_list
                            ),
                        }
                    )
            ...
            if not is_index_all_match:
                index_name, column_list = self.add_index(
                    _table['filter_column_list'], visitor.order_list
                )
                index_recommendation = self.add_index_return_format(
                    _table['table_name'], index_name, column_list
                )
                ...
                if not min_selectivity:
                    ...
                    new_index = self.format_index(
                        new_index,
                        _table['filter_column_list'],
                        visitor.projection_column_list,
                        visitor.order_list,
                        visitor.min_max_list,
                        _table['table_name'],
                    )
                    ...
                    new_index_selectivity = CBOOptimizer().calculate_selectivity(
                        new_index,
                        visitor,
                        _table['filter_column_list'],
                        _selectivity_dict,
                    ...
                    if not new_index_selectivity:
                        diagnosis_reason = (
                            'This is a better query range index , but due to lack of statistics, '
    ...
    def add_index(self, filter_column_list, order_list):
        column_list = MetaDataUtils.extension_all_match_index(
            filter_column_list, order_list
        )
        index_name = 'idx_sqless_{_index_name}'.format(
            _index_name='_'.join(column_list)
        )
        return index_name, column_list

    def add_index_return_format(self, table_name, index_name, column_list):
        return 'alter table {table_name} add index {index_name}({column_str})'.format(
    ...
    def get_diagnosis_reason_by_index(self, index_name, index_list):
        for _index in index_list:
            if _index.index_name == index_name:
                return (
                    'Query Range : {query_range} , Index Back : {is_index_back} , '
                    'Interesting Order : {interesting_order}'.format(
                        query_range=_index.extract_range,
    ...
    def format_index(
        ...
        extract_range = MetaDataUtils.extract_range(
            index.column_list, filter_column_list
        )
        index_all_match = MetaDataUtils.index_all_match(
            index.column_list, filter_column_list
        )
        has_interesting_order = MetaDataUtils.has_interesting_order(
            index.column_list,
            order_list,
            min_max_list,
            extract_range,
