...
"""
import json
from common.db_query import insert_user_optimization
from flask_restful import reqparse

from src.api.base_api import APIArgument, BaseAPI
from src.common.enum import OptimizationTypeEunm
...
class Optimizer(BaseAPI):
    ...
    def post(self):
        ...
        if catalog_json:
            catalog_object = (
                MetaDataUtils.json_to_catalog(catalog_json, schema_sql)
                if catalog_json
                else None
            )
        elif schema_sql:
        ...
        (
            ...
            after_sql_rewrite,
        ) = opt().optimize(self.sql_text, catalog_object)
        grade = 3
...
class Parse(BaseAPI):
    def __init__(self, *args, **kwargs):
        ...
        parser = reqparse.RequestParser(argument_class=APIArgument, bundle_errors=True)
        parser.add_argument(
            'sqlText', required=True, help="sqlText cannot be blank!", location='args'
        )
        parser.add_argument(
            'databaseEngine',
            required=True,
    ...
    def get(self):
        ...
        for _table in table_list:
            ...
            for column in filter_column_list:
                column_list.append(
                    {"tableName": table_name, "columnName": column['column_name']}
                )

        data = {"columnList": column_list, "tableList": temp_table_list}
        return self.construct_success_response_entity(data=data)
