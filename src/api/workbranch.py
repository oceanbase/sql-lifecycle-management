...
from common.db_query import (
    get_user_database,
    get_user_optimization,
    insert_user_database,
    read_user_optimization,
...
class Database(BaseAPI):
    ...
    def post(self):
        ...
        parser.add_argument(
            'databaseEngine',
            required=True,
            choices=['MySQL', 'OceanBase'],
            help="The databaseEngine only supports MySQL/OceanBase",
        )
        parser.add_argument(
            'databaseVersion', required=True, help="databaseVersion cannot be blank!"
        )
        ...
        args = parser.parse_args()
        is_insert_success, error_message = insert_user_database(
            self.user_id,
            args['databaseAlias'],
            args['databaseEngine'],
        ...
        if error_message:
            return self.construct_success_response_entity(
                message=error_message, success=is_insert_success
            )
        else:
    ...
    def patch(self):
        ...
        args = parser.parse_args()
        success, error_message = update_user_database(
            args['databaseAlias'],
            args['databaseEngine'],
            args['databaseVersion'],
            args['platform'],
        ...
        if error_message:
            return self.construct_success_response_entity(
                message=error_message, success=success
            )
        else:
...
class UserOptimization(BaseAPI):
    ...
    def get(self):
        ...
        parser.add_argument(
            'optimizationType',
            choices=['optimize', 'analysis', 'review'],
        ...
        optimization_list, total_count = get_user_optimization(
            self.user_id, args['startTimeTs'], args['endTimeTs']
        )
        ...
        for optimization in optimization_list:
            ...
            if 'reviewDetail' in report:
                for detail in report['reviewDetail']:
                    recommendation_list = detail['report'][
        ...
        data = {"optimizationHistoryList": optimization_list}
        return self.construct_success_response_entity(
