...
class MyTestCase(unittest.TestCase):
    ...
    def test_remove_now_in_insert(self):
        ...
        assert (
            sql
            == """insert ignore into bumonitor_risk_process_context (gmt_create, gmt_modified, rowkey, context) values (?, ?, ?, ?)"""
        )
    ...
    def test_hint(self):
        ...
        assert (
            sql
            == """select                  distinct(trace_id)             from                 midas_record_value where tenant=\'insttrade\' and is_expired=1  order by gmt_modified asc limit 500"""
        )
